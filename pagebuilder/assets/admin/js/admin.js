jQuery(document).ready(function($){
	...
	var toolbar_init = function(){
		...
		if ($('#wpadminbar').css('position') == 'fixed') {
			toolbar_vars.wp_toolbar_height += $('#wpadminbar:visible').height();
		}
		if (is_gutenberg_editor) {
			// WordPress5.5以上
			if ($('.interface-interface-skeleton__content').length) {
				toolbar_vars.scroll_parent = $('.interface-interface-skeleton__content').get(0);
				toolbar_vars.$scroll_parent = $('.interface-interface-skeleton__content');
				toolbar_vars.wp_toolbar_height += $('.edit-post-header').innerHeight();
			// WordPress5.5未満
			} else if ($('.edit-post-layout__content').length) {
				toolbar_vars.scroll_parent = $('.edit-post-layout__content').get(0);
				toolbar_vars.$scroll_parent = $('.edit-post-layout__content');
				toolbar_vars.wp_toolbar_height += $('.edit-post-header').innerHeight();
			} else {
				toolbar_vars.scroll_parent = false;
			}
		}
		toolbar_scroll()
	};
	var toolbar_scroll = function(){
		...
		if (is_gutenberg_editor) {
			if (!toolbar_vars.scroll_parent) return false;
	...
	var toolbar_scroll_on = function(){
		toolbar_init();
		$(window).on('resize.pb_toolbar_scroll', toolbar_init);
		if (is_gutenberg_editor) {
			if (!toolbar_vars.$scroll_parent) return false;

			clearTimeout(toolbar_vars.timer);
			toolbar_vars.timer = setTimeout(function(){
				toolbar_vars.$scroll_parent.on('scroll.pb_toolbar_scroll', toolbar_scroll);
			}, 100);
		} else {
			clearTimeout(toolbar_vars.timer);
			toolbar_vars.timer = setTimeout(function(){
				$(window).on('scroll.pb_toolbar_scroll', toolbar_scroll);
			}, 100);
		}
	};
	var toolbar_scroll_off = function(){
		toolbar_vars.status = '';
		$(window).off('resize.pb_toolbar_scroll');
		if (is_gutenberg_editor) {
			if (!toolbar_vars.$scroll_parent) return false;

			toolbar_vars.$scroll_parent.off('scroll.pb_toolbar_scroll');
		} else {
			$(window).off('scroll.pb_toolbar_scroll');
		}
	};
		...
		$('#editor').on('click', '.edit-post-more-menu button:first', function(){
			setTimeout(function(){
				...
				if ($pb_metabox.find('[name="use_page_builder"]').val() == 1) {
					// 24pxチェックあり
					if ($current_editor_button.html().indexOf('<svg width="24" height="24"') > -1) {
						$pb_select_button = $('<button type="button" aria-checked="true" role="menuitemradio" class="components-button components-menu-item__button components-menu-items-choice has-text has-icon"><svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="components-menu-items__item-icon" role="img" aria-hidden="true" focusable="false"><path d="M9 18.6L3.5 13l1-1L9 16.4l9.5-9.9 1 1z"></path></svg>' + pb_select_button_label + '</button>');
	...
	// 旧エディター
		...
		$('#wp-content-wrap .wp-editor-tabs').append('<span id="content-pagebuilder" class="hide-if-no-js wp-switch-editor switch-pagebuilder">' + $pb_metabox.find('.hndle').text() + '</span>');
	...
	$('.pb-add-row').on('click', function(){
		...
		$('#pb-add-row-modal').show().find('.pb-delete, .pb-row-clone').hide();
 ...
 + '</div>',
	...
	// モーダル内セレクト変更時のクローン対策
	$pb_metabox.on('change', '.pb-modal select', function(e){
		var v = $(this).val();
		$(this).find('option').removeAttr('selected');
	...
	$pb_metabox.on('change', '.pb-modal-row-edit select.cells', function(e){
		generateRowPreview($(this).closest('.pb-modal'), $(this).val());
	});
	...
	$('#pb-add-row-modal .pb-apply').on('click', function(e){
		...
		// 選択中カラムがあればその行の後ろに行追加
		var $selected_row = $rows.find('.cell.cell-selected').closest('.pb-row-container');
		if ($selected_row.length) {
			$selected_row.after($row.prop('outerHTML').replace(/rowindex/g, row_index));
		} else {
	...
	$pb_metabox.on('click', '.pb-rows-container .pb-row-container .pb-row-edit', function(e){
		...
		$pb_modal.show();
		$pb_modal.find('.pb-delete, .pb-row-clone').show();
	...
	// 行複製ボタン（モーダル内複製も共通）
	$pb_metabox.on('click', '.pb-rows-container .pb-row-container .pb-row-clone', function(e){
		// モーダルを閉じる
		$('.pb-modal:visible').hide();
		window_scroll_enable();

		var $rows = $(this).closest('.pb-rows-container');
		var $source_row = $(this).closest('.pb-row-container');
		var source_row_index = $source_row.find('.pb-row-index').val();

		// ウィジェットソータブル 停止
		$('.widgets-container').sortable('destroy')
		...
		$row_modal_clone.find(':input').not(':button, :submit').each(function(){
			// 複製元のinput取得
			var source_input_name = this.name.replace(/rowindex/g, source_row_index);
			var source_input = document.getElementsByName(source_input_name);
			if (source_input.length) {
				var $source_input = jQuery(document.getElementsByName(source_input_name)[0]);
				if (source_input.length > 1 && $(this).is(':radio, :checkbox')) {
					for(var i = 0; i < source_input.length; i++) {
						if (this.value === source_input[i].value) {
							$source_input = jQuery(document.getElementsByName(source_input_name)[i]);
						}
					}
				}
			} else {
			...
			if ($(this).is('.wp-color-picker')) {
				var $pickercontainer = $(this).closest('.wp-picker-container');
				var $clone = $(this).clone().attr('value', $source_input.val()).attr('data-value', $source_input.val());
				$pickercontainer.after($clone).remove();
			} else if ($(this).is('select')) {
				var v = $source_input.val();
				$(this).find('option').removeAttr('selected');
				$(this).find('[value="' + v + '"]').attr('selected', 'selected');
				$(this).attr('data-value', v);
			} else if ($(this).is(':radio, :checkbox')) {
				if ($source_input.is(':checked')) {
		...
		// 複製元行の後ろに行追加
		$source_row.after($row.prop('outerHTML').replace(/rowindex/g, row_index));
		$row = $('#row-' + row_index);
		...
		// 複製元カラムループ
		var $source_cells = $source_row.find('.pb-cells .cell');
		var $cells = $row.find('.pb-cells .cell');
		var $source_cell, $widgets_container;
		for(var i = 0; i < $source_cells.length; i++) {
			$source_cell = $source_cells.eq(i);
			$widgets_container = $cells.eq(i).find('.widgets-container');

			// 複製元ウィジェットループ
			$source_cell.find('.pb-widget').each(function(){
				//ウィジェット複製
				widget_clone($(this).attr('data-widget-index'), $widgets_container);
			});
		}
		...
		// 複製クラス追加削除
		$row.find('.pb-widget').addClass('pb-widget-cloned');
		setTimeout(function(){
			$row.find('.pb-widget').removeClass('pb-widget-cloned');
		}, 5000);

		// 行複製完了トリガー
		$row.trigger('page-builder-row-cloned');
	});
	...
	// ウィジェット複製処理
	var widget_clone = function(source_widget_index, $target_cell){
		// 複製元ウィジェット
		...
		$source_widget.find(':radio[data-checked]').removeAttr('data-checked');
		...
		$widget.attr('data-clone-source-widget-index', source_widget_index)
		...
		// ウィジェット複製完了トリガー
		$pb_metabox.trigger('page-builder-widget-cloned', [$widget, $source_widget]);
