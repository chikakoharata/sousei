 ...
 * ページビルダーバージョン
 */
define('PAGE_BUILDER_VERSION', '1.4.3');
...
function show_page_builder_metabox() {
	...
	echo '<span class="pb-tool-button pb-add-widget" data-require-row-message="'. __('Please click Add block button to start page builder.', 'tcd-w').'"><span class="dashicons dashicons-plus"></span> '. __('Add content', 'tcd-w') . '</span>';
	echo '<span class="pb-tool-button pb-add-row"><span class="dashicons dashicons-grid-view"></span> '. __('Add block', 'tcd-w'). '</span>';
	echo '</div>';
	echo "\n";

	// rows
	echo '<div class="pb-rows-container" data-rows="'.$row_index_max.'" data-widgets="'.$widget_index_max.'" data-require-row-message="'. __('Please click Add block button to start page builder.', 'tcd-w').'">';
	...
	render_page_builder_edit_row_modal(array(
		'index' => 'rowindex',
		'id' => 'pb-add-row-modal',
		'title' => __('Add block', 'tcd-w'),
		'primary_button_label' => __('Add', 'tcd-w'),
		'attribute' => 'data-edit-title="'.esc_attr__('Block setting', 'tcd-w').'" data-edit-button="'.esc_attr__('Done', 'tcd-w').'"'
	));
...
<div id="<?php echo esc_attr($args['id']); ?>" class="pb-row-container">
	...
	<div class="pb-row-toolbar">
		<span class="pb-tool-button pb-row-edit"><?php _e('Edit block', 'tcd-w'); ?></span>
		<span class="pb-tool-button pb-row-clone"><?php _e('Clone block', 'tcd-w'); ?></span>
		<span class="pb-tool-button pb-row-delete" data-confirm="<?php _e("Do you want to delete this block?\nIf you delete this block, registered contents will also be deleted.", 'tcd-w'); ?>"><?php _e('Delete block', 'tcd-w'); ?></span>
	...
	<div class="pb-toolbar">
		...
		<div class="pb-buttons">
			<div class="action-buttons">
				<a class="pb-delete" data-confirm="<?php _e("Do you want to delete this block?\nIf you delete this block, registered contents will also be deleted.", 'tcd-w'); ?>"><?php _e('Delete', 'tcd-w'); ?></a>
				<a class="pb-row-clone"><?php _e('Clone', 'tcd-w'); ?></a>
			</div>
	...
	<div class="pb-sidebar pb-right-sidebar">
		...
		<div class="pb-toggle-content pb-right-sidebar-layout">
			<div class="form-field">
				<label><?php _e('Margin bottom of the block', 'tcd-w'); ?></label>
				...
				<p class="pb-description"><?php _e('Space below the block.<br />Default is 30px.', 'tcd-w'); ?></p>
			</div>

			<div class="form-field">
				<label><?php _e('Margin bottom of the block for mobile', 'tcd-w'); ?></label>
			...
			<div class="form-field">
				<label><?php _e('Block width', 'tcd-w'); ?></label>
				...
				<p class="pb-description"><?php _e('Please register the <strong>px</strong> of width if you want to change the width of this block.<br /><br />If you registered the width, the block will be center aligned.', 'tcd-w'); ?></p>
			</div>
			...
			<div class="form-field form-field-checkbox">
				...
				<label class="checkbox"><input type="checkbox" name="pagebuilder[row][<?php echo esc_attr($args['index']); ?>][nextpage]" value="1" <?php if ($args['nextpage'] == 1) echo 'checked="checked"'; ?> /> <?php _e('Display this block at next page.', 'tcd-w'); ?></label>
			</div>
		</div>
...
/**
 * フロント用 pre_handle_404フィルター
 * WordPress5.5のページ分割時2ページ目以降のリクエストで本文側に<!--nextpage-->がページ番号数以上存在しないと1ページ目にリダイレクトされる対策
 */
function page_builder_filter_pre_handle_404($false, $wp_query) {
	global $wp_version, $pages;

	if (version_compare($wp_version, '5.5', '>=' ) && !empty($wp_query->query_vars['page'])) {
		if ($wp_query->is_singular() && !empty($wp_query->post->ID) && is_page_builder($wp_query->post)) {
			render_page_builder_content($wp_query->post->ID);
			if ($pages && count($pages) >= $wp_query->query_vars['page']) {
				return true;
			}
		}
	}
...
add_filter('pre_handle_404', 'page_builder_filter_pre_handle_404', 10, 2);
